/**
 * This app is deployed behind a class-provided gateway. Two paths on that
 * gateway were tried:
 *   - Claude directly (api.anthropic.com shape) — blocked for student keys (403).
 *   - Claude via the gateway's OpenRouter route (anthropic/claude-3.5-sonnet)
 *     — reachable, but rejected because no price is published for that model
 *     on this gateway's account.
 * What's actually configured and verified working is the gateway's
 * OpenAI-compatible Chat Completions endpoint using a gpt-5.6 model. See
 * /notes for the full story — this app calls that model, not Claude.
 */
const GATEWAY_BASE_URL =
  process.env.GATEWAY_BASE_URL?.replace(/\/+$/, "") ?? "https://174.138.16.223";
const CHAT_COMPLETIONS_URL = `${GATEWAY_BASE_URL}/v1/chat/completions`;
const MODEL = process.env.GATEWAY_MODEL ?? "gpt-5.6-terra";
...
/**
 * Calls the configured model through the gateway's OpenAI-compatible Chat
...
export async function callModel({
  ...
  user,
  maxTokens = 1024,
}: ModelCallOptions): Promise<ModelResult> {
